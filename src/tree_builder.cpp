...
SubhaloPtr TreeBuilder::define_central_subhalo(HaloPtr &halo, SubhaloPtr &subhalo, SimulationParameters &sim_params,
	       DarkMatterHaloParameters &dark_matter_params, const DarkMatterHalosPtr &darkmatterhalos, bool z0)
{
	...
	if(dark_matter_params.apply_fix_to_mass_swapping_events){
		...
		// redefine only at z=0 (Riera, and when the values at z=0 are passed to the main progenitors: log-normal distribution or from catalogues for subhalos with few particles)
		// or at any redshift when we use the values from catalogues for subhalos with enough particle number
		if((z0 && dark_matter_params.riera_spin_evolution) || (z0 && (!dark_matter_params.use_converged_lambda_catalog || (dark_matter_params.use_converged_lambda_catalog && subhalo->Mvir/sim_params.particle_mass < dark_matter_params.min_part_convergence))) || (dark_matter_params.use_converged_lambda_catalog && subhalo->Mvir/sim_params.particle_mass >= dark_matter_params.min_part_convergence)){
	...
	// halo spin corresponds to central subhalo since angular momentum in the catalogues comes from subhalo data
	...
	omp_static_for(trees, threads, [&](const MergerTreePtr &tree, unsigned int thread_idx) {
		for (int snapshot=sim_params.max_snapshot; snapshot >= sim_params.min_snapshot; snapshot--) {
			...
			for (auto &halo: tree->halos_at(snapshot)) {
				...
				auto lambda = subhalo->lambda; //darkmatterhalos->halo_lambda(*subhalo, halo->Mvir, z, npart); // z=0 (or at least the halo last snapshot) value for spin
				...
				while (!ascendants.empty()) {
					...
					if ((!dark_matter_params.use_converged_lambda_catalog || (dark_matter_params.use_converged_lambda_catalog && main_prog->Mvir/sim_params.particle_mass < dark_matter_params.min_part_convergence)) && !dark_matter_params.riera_spin_evolution) {
						main_prog->lambda = lambda;
					}
					// Use Riera spin evolution (for the Peebles or Bullock spin parameter)
					else if(dark_matter_params.riera_spin_evolution){
					        main_prog->lambda = darkmatterhalos->riera_spin_evolution(lambda, z, sim_params.redshifts[main_prog->snapshot]);
					}
